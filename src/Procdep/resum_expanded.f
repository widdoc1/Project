      double precision function resum_expanded(r,wgt)
      implicit none
      include 'constants.f'
      include 'noglue.f'
...
!$omp threadprivate(nshot,/useropt2/)
...
c      write(6,*) 'resum_expanded: j,k,msqv=',j,k,fx1(j)*fx2(k)*msqv(j,k)
      if (j .eq. k) then
        m=+1
        n=+1
        csmax=6
     ...
     & +msq_struc(cs,j,g)*(AP(g,q,2)+H2(g,q,q,cs,2)))*fx1(j)*fx2z(k)/z
      enddo      
      xmsq=xmsq+xmsqt
c      write(6,*) 'resum_expanded: j,k,  ct=',j,k,xmsqt
...
c      write(6,*) 'resum_expanded: j,k,msqv=',j,k,fx1(j)*fx2(k)*msqv(j,k)
     ...
     & +msq_struc(iqr,m,-m)*(AP(a,g,2)+H2(a,g,q,iqr,2))*fx1(j)*fx2z(g)/z
      xmsq=xmsq+xmsqt
c      write(6,*) 'resum_expanded: j,k,  ct=',j,k,xmsqt
c      write(6,*) 'resum_expanded: j,k, SUM=',j,k,xmsqt+fx1(j)*fx2(k)*msqv(j,k)
...
c---  SSend                                                                                                                        
 ...
 20   continue
      ...
      if (currentPDF .eq. 0) then
        resum_expanded=flux*xjac*pswt*xmsq/BrnRat
      endif
...
!$omp atomic
     ...
     .       val*xmsq_bypart(j,k)
      ...
      val=resum_expanded*wgt 
      val2=val**2 
c---  SSbegin
      resum_expanded = resum_expanded*reweight
c---  SSend
...
c--- handle special caase of Qflag and Gflag
      if (QandGflag) then
        QandGint=QandGint+resum_expanded
...
c--- return both to .true. and assign value to resum_expanded (to return to VEGAS)
        Qflag=.true.
        Gflag=.true.
        resum_expanded=QandGint
      endif
      endif
