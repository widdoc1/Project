...
c--- quark-quark or antiquark-antiquark
      if (  ((j > 0).and.(k > 0))
     & .or. ((j < 0).and.(k < 0))) then
c      write(6,*) 'expdNNLLint: j,k,msqv=',j,k,fx1(j)*fx2(k)*msqv(j,k)
      if (j == k) then
        m=+1
        n=+1
        csmax=6
     ...
     & +msq_struc(cs,j,g)*(AP(g,q,2)+H2(g,q,q,cs,2)))*fx1(j)*fx2z(k)/z
      enddo      
      xmsq=xmsq+xmsqt
c      write(6,*) 'expdNNLLint: j,k,  ct=',j,k,xmsqt
...
c      write(6,*) 'expdNNLLint: j,k,msqv=',j,k,fx1(j)*fx2(k)*msqv(j,k)
     ...
     & +msq_struc(iqr,m,-m)*(AP(a,g,2)+H2(a,g,q,iqr,2))*fx1(j)*fx2z(g)/z
      xmsq=xmsq+xmsqt
c      write(6,*) 'expdNNLLint: j,k,  ct=',j,k,xmsqt
c      write(6,*) 'expdNNLLint: j,k, SUM=',j,k,xmsqt+fx1(j)*fx2(k)*msqv(j,k)
...
c---  SSend                                                                                                                        
 ...
 20   continue
      ...
      if (currentPDF == 0) then
        expdNNLLint=flux*xjac*pswt*xmsq/BrnRat
...
!$omp atomic
     ...
     &       val*xmsq_bypart(j,k)
      ...
      if(expdNNLLint.ne.expdNNLLint) then
         write(6,*) 'Found expdNNLLint=',expdNNLLint
         write(6,*) 'random #s',r
         expdNNLLint=zip
         goto 999
      endif
...
c---  SSbegin
      expdNNLLint = expdNNLLint*reweight
c---  SSend
...
c--- return both to .true. and assign value to expdNNLLint (to return to VEGAS)
        Qflag=.true.
        Gflag=.true.
        expdNNLLint=QandGint
      endif
      endif
