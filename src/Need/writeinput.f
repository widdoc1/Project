...
c--- catch special scale choices for stop+b process
      ...
      if ((tag .eq. 'resumscale') .or. writeall) then
      write(unitno,fmt=f99) initresumscale,'resumscale'
      endif

      if ((tag .eq. 'dynamicscale') .or. writeall) then
      write(unitno,fmt=f96) dynstring,'dynamicscale'
      endif
c      if ((tag .eq. 'fdynamicscale') .or. writeall) then
c      write(unitno,fmt=f96) fdynstring,'fdynamicscale'
c      endif
     ...
     . lstring//' [Coupling rescaling in kappa formalism] )'
      ...
      if ((tag .eq. 'k_t') .or. (writeall)) then
      write(unitno,fmt=f99) k_t,'k_t'
      endif
      if ((tag .eq. 'k_b') .or. (writeall)) then
      write(unitno,fmt=f99) k_b,'k_b'
      endif
      if ((tag .eq. 'k_g') .or. (writeall)) then
      write(unitno,fmt=f99) k_g,'k_g'
      endif
